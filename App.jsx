  ...
  return (
    <main style={{ background: C.sand, color: C.charcoal }}>
      ...
      <section style={{ padding: '0 clamp(20px, 4vw, 56px) 100px' }}>
        <Reveal>
          <div style={{ position: 'relative', maxWidth: 900, margin: '0 auto', background: C.charcoal, borderRadius: 12, overflow: 'hidden', boxShadow: '0 30px 80px rgba(15,15,16,0.25)' }}>
            <video
              src={c.video}
              controls
              playsInline
              style={{ width: '100%', display: 'block', aspectRatio: '9/16', maxHeight: '85vh', background: '#000', objectFit: 'contain' }}
            />
          </div>
        </Reveal>
        <Reveal delay={140}>
          <div style={{ textAlign: 'center', marginTop: 18, fontSize: 12, color: `${C.charcoal}88`, letterSpacing: '0.2em', fontWeight: 700 }}>
            CGI FILM  ·  PRODUCED WITH @WESUALIZESTUDIOS
          </div>
        </Reveal>
      </section>
            ...
            <Reveal key={oc.slug} delay={i * 120}>
              ...
              >
                <div style={{ position: 'relative', height: 320, overflow: 'hidden', marginBottom: 16, background: `linear-gradient(135deg, ${oc.accent} 0%, ${oc.accent2} 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ fontSize: 96, fontWeight: 700, color: `${C.white}22`, letterSpacing: '-0.04em' }}>
                    {String(CASES.findIndex((cc) => cc.slug === oc.slug) + 1).padStart(2, '0')}
                  </div>
                  <div style={{ position: 'absolute', top: 16, left: 16, display: 'flex', gap: 6 }}>
                    {oc.tags.map((t) => (
